...
/*
 * Los límites de tamaño viven en config/media.php, que es el único sitio donde
 * se declaran. Se leen con require en vez de con config() porque este fichero
 * se evalúa mientras la configuración se está construyendo, y el orden en que
 * Laravel carga los ficheros no es algo sobre lo que convenga apoyarse.
 */
$limites = require __DIR__.'/media.php';

return [

    /*
     * Disco donde se guardan los archivos subidos desde el panel: portadas y PDF
     * de la revista, logo, imágenes de comunicados y blog, retratos docentes.
     *
     * El portal admite DOS infraestructuras sin cambiar una línea de código,
     * porque no sabemos cuál usará el área de TI de la universidad:
     *
     *   MEDIA_DISK=public  → disco del servidor, en storage/app/public.
     *                        Exige un volumen persistente montado ahí y el
     *                        enlace public/storage (lo crea el entrypoint).
     *   MEDIA_DISK=medios  → proveedor compatible con S3 (R2, Neon, MinIO,
     *                        AWS S3, un servidor propio). Exige las AWS_*.
     *
     * Véase config/filesystems.php y la sección «Almacenamiento» de DEPLOY.md.
     *
     * Este fichero está publicado entero a propósito. Un fichero parcial
     * funcionaría en local, pero no en producción: mergeConfigFrom() se salta
     * la fusión con los valores del paquete cuando la configuración está
     * cacheada, y el entrypoint ejecuta config:cache.
     */
     ...
     *
     * Se deriva de config/media.php en vez de fijarse aquí porque tres capas
     * distintas limitan una misma subida y deben decir lo mismo: PHP
     * (docker/uploads.ini), el campo de Filament (maxSize) y esta. El valor
     * original del paquete eran 10 MB, la mitad que las otras dos: un PDF de
     * 15 MB superaba el navegador y el formulario, y moría aquí con
     * FileIsTooBig después de haberse transferido entero.
     */
    'max_file_size' => 1024 * max($limites['max_pdf_kb'], $limites['max_image_kb']),
