...
@section('content')
    <article class="mx-auto max-w-3xl px-6 py-16 lg:py-24">
        <a href="{{ route('revista.numero', $numero) }}" wire:navigate.hover class="inline-flex items-center gap-2 text-sm font-semibold text-navy-700"><x-ui-icon name="arrow-left" /> Volver al número</a>
        ...
        <div class="mt-3 flex flex-wrap gap-4 text-sm text-stone-500">
            @if($articulo->fecha)<time>{{ $articulo->fecha->translatedFormat('d \d\e F \d\e Y') }}</time>@endif
