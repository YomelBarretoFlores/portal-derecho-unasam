...
<article class="card-hover flex h-full flex-col rounded-none border border-stone-200 bg-white p-7">
    ...
    <h3 class="mt-4 text-xl font-semibold leading-snug text-navy-900">
        @if ($numeroSlug)
            <a href="{{ route('revista.articulo', [$numeroSlug, $articulo->slug]) }}" wire:navigate.hover class="transition hover:text-navy-600">{{ $articulo->titulo }}</a>
        @else
            {{ $articulo->titulo }}
        @endif
    </h3>
