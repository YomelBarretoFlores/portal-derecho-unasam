...
@section('content')
    ...
    <section class="mx-auto max-w-4xl px-6 py-20">
        ...
        <div class="reveal grid gap-4 sm:grid-cols-3">
            @foreach ([['Grado académico', $grado], ['Título profesional', $tituloProf], ['Modalidad', $modalidad]] as [$k, $v])
                <div class="rounded-2xl border border-stone-200 bg-paper p-5 text-center">
                    <p class="font-sans text-xs uppercase tracking-wide text-stone-400">{{ $k }}</p>
        ...
        @if ($ciclos->isNotEmpty())
            <div class="reveal mt-12" data-reveal-delay="0.05">
                ...
                <div class="mt-6 space-y-4" x-data="{ abierto: 1 }">
                                        ...
                                        @foreach ($cursos as $curso)
                                            <tr>
                                                ...
                                                <td class="px-5 py-3 text-right">
                                                    @if ($curso->tipo)
                                                        <span class="rounded-full bg-navy-50 px-2.5 py-0.5 text-[11px] font-medium text-navy-700">{{ $curso->tipo }}</span>
                                                    @endif
                                                </td>
            ...
            <div class="mt-6 space-y-3">
                ...
                @if ($sgaUrl)
                    <a href="{{ $sgaUrl }}" target="_blank" rel="noopener"
                       class="card-hover flex items-center gap-4 rounded-2xl border border-stone-200 bg-white p-5">
                        <span class="flex h-12 w-12 items-center justify-center rounded-xl bg-stone-50 text-blue-600">
